...
@section('content')
<!-- Page Header -->
<section class="py-16 bg-gradient-to-r from-{{ $region->name == 'Jawa Barat' ? 'blue' : ($region->name == 'Jawa Tengah' ? 'green' : 'purple') }}-600 to-{{ $region->name == 'Jawa Barat' ? 'blue' : ($region->name == 'Jawa Tengah' ? 'green' : 'purple') }}-700 text-white">
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-xl opacity-90 max-w-3xl">
                    Informasi detail kebun model teh di wilayah {{ $region->name }} dengan strategi produksi yang optimal
                </p>
            </div>
            <div class="hidden md:block">
                <span class="material-icons text-6xl opacity-20">{{ $region->name == 'Jawa Barat' ? 'terrain' : ($region->name == 'Jawa Tengah' ? 'agriculture' : 'park') }}</span>
            </div>
        </div>
    </div>
</section>
                ...
                <div class="space-y-4 text-gray-700">
                    <p class="leading-relaxed">
                        @if($region->name == 'Jawa Barat')
                            Wilayah Jawa Barat merupakan pusat utama pengembangan kebun model teh dengan tiga lokasi strategis yang mewakili berbagai ketinggian dan kondisi mikroklimat. Kawasan ini dikenal dengan tanah vulkanik yang subur dan curah hujan tinggi yang ideal untuk budidaya tanaman teh.
                        @elseif($region->name == 'Jawa Tengah')
                            Jawa Tengah melalui kebun model Kaligua menyediakan representasi unik untuk budidaya teh di dataran rendah. Wilayah ini penting untuk studi adaptasi perubahan iklim dan pengembangan varietas toleran panas.
                        @else
                            Sumatra Selatan dengan kebun model Pagar Alam menawarkan ekosistem pegunungan yang unik dengan tanah vulkanik eksotis. Wilayah ini menjadi penting untuk diversifikasi genetik dan studi komparatif dengan Jawa.
                        @endif
                    </p>
                    <div class="grid grid-cols-2 gap-4 mt-6">
                        ...
                        <div class="bg-gray-50 p-4 rounded-lg">
                            ...
                            <p class="text-2xl font-bold text-blue-600">{{ number_format($region->gardens->sum('area_hectares'), 1) }} ha</p>
                        </div>
                    </div>
                </div>
            </div>
            <div class="relative">
                <img src="https://trae-api-sg.mchost.guru/api/ide/v1/text_to_image?prompt={{ urlencode($region->name . ' tea plantation landscape, beautiful scenery, professional photography, lush green tea gardens') }}&image_size=landscape_4_3" 
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            @foreach($region->gardens as $garden)
                <div class="pptk-card p-6 group hover:border-{{ $region->name == 'Jawa Barat' ? 'blue' : ($region->name == 'Jawa Tengah' ? 'green' : 'purple') }}-500 transition-all duration-300">
                    <div class="relative overflow-hidden rounded-lg mb-4">
                        <img src="https://trae-api-sg.mchost.guru/api/ide/v1/text_to_image?prompt={{ urlencode($garden->name . ' tea garden, beautiful plantation, professional photography, lush green tea bushes') }}&image_size=landscape_4_3" 
                             alt="Kebun {{ $garden->name }}" 
                    ...
                    <div class="space-y-2 mb-4">
                        <div class="flex items-center text-gray-700">
                            <span class="material-icons text-{{ $region->name == 'Jawa Barat' ? 'blue' : ($region->name == 'Jawa Tengah' ? 'green' : 'purple') }}-600 mr-2 text-sm">location_on</span>
                            <span class="text-sm">{{ $garden->location }}</span>
                        </div>
                        <div class="flex items-center text-gray-700">
                            <span class="material-icons text-{{ $region->name == 'Jawa Barat' ? 'blue' : ($region->name == 'Jawa Tengah' ? 'green' : 'purple') }}-600 mr-2 text-sm">straighten</span>
                            <span class="text-sm">{{ number_format($garden->area_hectares, 1) }} hektar</span>
                        </div>
                        <div class="flex items-center text-gray-700">
                            <span class="material-icons text-{{ $region->name == 'Jawa Barat' ? 'blue' : ($region->name == 'Jawa Tengah' ? 'green' : 'purple') }}-600 mr-2 text-sm">calendar_today</span>
                            <span class="text-sm">Didirikan {{ \Carbon\Carbon::parse($garden->established_at)->format('Y') }}</span>
                        </div>
                    </div>
                    
                    <a href="{{ route('strategic.garden', ['region' => $region->id, 'garden' => $garden->id]) }}" 
                       class="block w-full text-center bg-gradient-to-r from-{{ $region->name == 'Jawa Barat' ? 'blue' : ($region->name == 'Jawa Tengah' ? 'green' : 'purple') }}-600 to-{{ $region->name == 'Jawa Barat' ? 'blue' : ($region->name == 'Jawa Tengah' ? 'green' : 'purple') }}-700 text-white py-2 rounded-lg font-medium hover:from-{{ $region->name == 'Jawa Barat' ? 'blue' : ($region->name == 'Jawa Tengah' ? 'green' : 'purple') }}-700 hover:to-{{ $region->name == 'Jawa Barat' ? 'blue' : ($region->name == 'Jawa Tengah' ? 'green' : 'purple') }}-800 transition-all duration-300">
                        Lihat Detail
                    </a>
                </div>
            @endforeach
