...
@section('content')
    <div class="max-w-4xl mx-auto bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
        <div class="mb-6">
            ...
            <h2 class="text-xl font-semibold text-gray-800 dark:text-gray-100">Form Edit Afdeling</h2>
        </div>
        ...
        <form action="{{ route('admin.afdelings.update', $afdeling) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="col-span-2">
                    ...
                    <select name="garden_id" id="garden_id"
                        ...
                        @foreach ($gardens as $garden)
                            <option value="{{ $garden->id }}" {{ old('garden_id', $afdeling->garden_id) == $garden->id ? 'selected' : '' }}>
                                {{ $garden->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-span-2 md:col-span-1">
                    ...
                    <input type="text" name="manager_name" id="manager_name" value="{{ old('manager_name', $afdeling->manager_name) }}"
                        class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 shadow-sm focus:border-green-500 focus:ring focus:ring-green-200">
                    @error('manager_name')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="total_area_ha" id="total_area_ha" value="{{ old('total_area_ha', $afdeling->total_area_ha) }}"
                        class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 shadow-sm focus:border-green-500 focus:ring focus:ring-green-200"
                        required>
                    @error('total_area_ha')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="tm_area_ha" id="tm_area_ha" value="{{ old('tm_area_ha', $afdeling->tm_area_ha) }}"
                        class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 shadow-sm focus:border-green-500 focus:ring focus:ring-green-200"
                        required>
                    @error('tm_area_ha')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
