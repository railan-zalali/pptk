...
@section('content')
    <div class="max-w-4xl mx-auto bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8 transition-all duration-300">
        <div class="mb-6 pb-6 border-b border-gray-100 dark:border-gray-700">
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Perbarui informasi blok yang sudah ada.</p>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="relative">
                        ...
                        <select name="plant_type" id="plant_type"
                            ...
                            <option value="seedling"
                                {{ old('plant_type', $block->plant_type) == 'seedling' ? 'selected' : '' }}>Seedling
                            </option>
                            <option value="klon_gmb"
                                {{ old('plant_type', $block->plant_type) == 'klon_gmb' ? 'selected' : '' }}>Klon GMB
                            </option>
                            <option value="klon_tri"
                                {{ old('plant_type', $block->plant_type) == 'klon_tri' ? 'selected' : '' }}>Klon TRI
                            </option>
                        </select>
                    </div>
                    @error('plant_type')
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="planting_year" id="planting_year"
                            value="{{ old('planting_year', $block->planting_year) }}" min="1900"
                            max="{{ date('Y') + 1 }}"
                            class="w-full pl-9 pr-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-green-500/20 focus:border-green-500 transition-colors"
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <select name="initial_class" id="initial_class"
                            ...
                            <option value="A"
                                {{ old('initial_class', $block->initial_class) == 'A' ? 'selected' : '' }}>A
                            </option>
                            <option value="B"
                                {{ old('initial_class', $block->initial_class) == 'B' ? 'selected' : '' }}>B
                            </option>
                            <option value="C"
                                {{ old('initial_class', $block->initial_class) == 'C' ? 'selected' : '' }}>C
                            </option>
                            <option value="D"
                                {{ old('initial_class', $block->initial_class) == 'D' ? 'selected' : '' }}>D
                            </option>
                            <option value="E"
                                {{ old('initial_class', $block->initial_class) == 'E' ? 'selected' : '' }}>E
                            </option>
                        </select>
                    </div>
                    @error('initial_class')
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <select name="topography" id="topography"
                            ...
                            <option value="datar"
                                {{ old('topography', $block->topography) == 'datar' ? 'selected' : '' }}>
                                Datar</option>
                            <option value="gelombang"
                                {{ old('topography', $block->topography) == 'gelombang' ? 'selected' : '' }}>Gelombang
                            </option>
                            <option value="curam"
                                {{ old('topography', $block->topography) == 'curam' ? 'selected' : '' }}>
                                Curam</option>
                        </select>
                    </div>
                    @error('topography')
