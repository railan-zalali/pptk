...
@section('content')
    <div class="max-w-4xl mx-auto bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
        <div class="mb-6">
            ...
            <h2 class="text-xl font-semibold text-gray-800 dark:text-gray-100">Form Edit Blok</h2>
        </div>
        ...
        <form action="{{ route('admin.blocks.update', $block) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="col-span-2">
                    ...
                    <select name="afdeling_id" id="afdeling_id"
                        ...
                        @foreach ($afdelings as $afdeling)
                            <option value="{{ $afdeling->id }}" {{ old('afdeling_id', $block->afdeling_id) == $afdeling->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="text" name="code" id="code" value="{{ old('code', $block->code) }}"
                        class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 shadow-sm focus:border-green-500 focus:ring focus:ring-green-200"
                        required>
                    @error('code')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $block->name) }}"
                        class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 shadow-sm focus:border-green-500 focus:ring focus:ring-green-200"
                        required>
                    @error('name')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <select name="plant_type" id="plant_type"
                        ...
                        <option value="seedling" {{ old('plant_type', $block->plant_type) == 'seedling' ? 'selected' : '' }}>Seedling</option>
                        <option value="klon_gmb" {{ old('plant_type', $block->plant_type) == 'klon_gmb' ? 'selected' : '' }}>Klon GMB</option>
                        <option value="klon_tri" {{ old('plant_type', $block->plant_type) == 'klon_tri' ? 'selected' : '' }}>Klon TRI</option>
                    </select>
                    @error('plant_type')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" name="planting_year" id="planting_year" value="{{ old('planting_year', $block->planting_year) }}"
                ...
                <div>
                    ...
                    <select name="initial_class" id="initial_class"
                        ...
                        <option value="A" {{ old('initial_class', $block->initial_class) == 'A' ? 'selected' : '' }}>A</option>
                        <option value="B" {{ old('initial_class', $block->initial_class) == 'B' ? 'selected' : '' }}>B</option>
                        <option value="C" {{ old('initial_class', $block->initial_class) == 'C' ? 'selected' : '' }}>C</option>
                        <option value="D" {{ old('initial_class', $block->initial_class) == 'D' ? 'selected' : '' }}>D</option>
                        <option value="E" {{ old('initial_class', $block->initial_class) == 'E' ? 'selected' : '' }}>E</option>
                    </select>
                    @error('initial_class')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="topography" id="topography"
                        ...
                        <option value="datar" {{ old('topography', $block->topography) == 'datar' ? 'selected' : '' }}>Datar</option>
                        <option value="gelombang" {{ old('topography', $block->topography) == 'gelombang' ? 'selected' : '' }}>Gelombang</option>
                        <option value="curam" {{ old('topography', $block->topography) == 'curam' ? 'selected' : '' }}>Curam</option>
                    </select>
                    @error('topography')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
