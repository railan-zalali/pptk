        ...
        <form action="{{ route('admin.visits.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <input type="date" name="visit_date" class="w-full border rounded px-3 py-2" required value="{{ old('visit_date', date('Y-m-d')) }}">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Durasi (jam) *</label>
                    <input type="number" name="duration" min="1" max="24" class="w-full border rounded px-3 py-2" required value="{{ old('duration', 2) }}">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Jumlah Peserta *</label>
                    <input type="number" name="participants_count" min="1" class="w-full border rounded px-3 py-2" required value="{{ old('participants_count', 5) }}">
                </div>
                <div>
            ...
            <div class="mb-6">
                ...
                <textarea name="participants_list" rows="3" class="w-full border rounded px-3 py-2">{{ old('participants_list') }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                <div>
                    ...
                    <textarea name="objectives" rows="3" class="w-full border rounded px-3 py-2">{{ old('objectives') }}</textarea>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Temuan</label>
                    <textarea name="findings" rows="3" class="w-full border rounded px-3 py-2">{{ old('findings') }}</textarea>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Rekomendasi</label>
                    <textarea name="recommendations" rows="3" class="w-full border rounded px-3 py-2">{{ old('recommendations') }}</textarea>
                </div>
            </div>
