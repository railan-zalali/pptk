    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6 border border-gray-200 dark:border-gray-700">
        <div class="mb-6 border-b border-gray-200 dark:border-gray-700 pb-4">
            <h2 class="text-xl font-bold text-gray-800 dark:text-gray-100 flex items-center gap-2">
                ...
                Edit Data Kebun — {{ $garden->kebun_name }}
            </h2>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Perbarui informasi kebun di sini.</p>
        </div>
        ...
        <form action="{{ route('admin.gardens.update', $garden) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input name="kebun_name" type="text" required
                            ...
                            value="{{ old('kebun_name', $garden->kebun_name) }}">
                    </div>
                    @error('kebun_name')<p class="text-red-600 text-xs mt-1">{{ $message }}</p>@enderror
                </div>
                    ...
                    <div class="relative">
                        ...
                        <select name="regional_id" required
                            ...
                            @foreach ($regions as $region)
                                <option value="{{ $region->id }}" {{ old('regional_id', $garden->regional_id) == $region->id ? 'selected' : '' }}>{{ $region->regional_name }}</option>
                            @endforeach
                        </select>
                    </div>
                    @error('regional_id')<p class="text-red-600 text-xs mt-1">{{ $message }}</p>@enderror
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input name="luas_total_ha" type="number" step="0.01" min="0.01" required
                            ...
                            value="{{ old('luas_total_ha', $garden->luas_total_ha) }}">
                    </div>
                    @error('luas_total_ha')<p class="text-red-600 text-xs mt-1">{{ $message }}</p>@enderror
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <select name="kebun_type" required
                            ...
                            <option value="Model" {{ old('kebun_type', $garden->kebun_type) == 'Model' ? 'selected' : '' }}>Model</option>
                            <option value="Pengembangan" {{ old('kebun_type', $garden->kebun_type) == 'Pengembangan' ? 'selected' : '' }}>Pengembangan</option>
                        </select>
                    </div>
                </div>
                ...
                @php
                    // Parse stored location: "Provinsi, Kab/Kota" or just the city
                    $storedLocation = old('location', $garden->location ?? '');
                    $storedProvince = old('province', '');
                    $storedCity     = $storedLocation;
                    if (!$storedProvince) {
                        // Try to detect province from stored location (format "Provinsi, City")
                        foreach (array_keys($provinces) as $prov) {
                            if (str_starts_with($storedLocation, $prov . ', ')) {
                                $storedProvince = $prov;
                                $storedCity     = substr($storedLocation, strlen($prov) + 2);
                                break;
                            }
                        }
                    }
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <select name="province" id="province-select"
                            ...
                            @foreach (array_keys($provinces) as $prov)
                                <option value="{{ $prov }}" {{ $storedProvince == $prov ? 'selected' : '' }}>{{ $prov }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input name="established_at" type="date"
                            ...
                            value="{{ old('established_at', $garden->established_at ? $garden->established_at->format('Y-m-d') : '') }}">
                    </div>
                    @error('established_at')<p class="text-red-600 text-xs mt-1">{{ $message }}</p>@enderror
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="3"
                        class="w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-green-500/20 focus:border-green-500 transition-colors">{{ old('description', $garden->description) }}</textarea>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="agro_climate_note" rows="3"
                        class="w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-green-500/20 focus:border-green-500 transition-colors">{{ old('agro_climate_note', $garden->agro_climate_note) }}</textarea>
                </div>
                ...
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                        ...
                        Koordinat Lokasi
                        <span class="ml-1 text-xs text-gray-400 font-normal">(klik peta untuk memindahkan pin)</span>
                    </label>
                    ...
                    <div class="flex gap-3">
                        <div class="relative flex-1">
                            ...
                            <input name="coordinates" id="coordinates-input" type="text"
                                ...
                                value="{{ old('coordinates', $garden->coordinates) }}"
                    ...
                    @error('coordinates')<p class="text-red-600 text-xs mt-1">{{ $message }}</p>@enderror
                    <p class="text-xs text-gray-400 mt-1">Format: <code>latitude, longitude</code>. Contoh: <code>-7.053, 107.641</code></p>
                </div>

                <!-- Photo -->
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Foto Kebun</label>
                    @if ($garden->photo_path)
                        <div class="mb-3">
                            <img src="{{ asset('storage/' . $garden->photo_path) }}" alt="Foto Kebun"
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Format: JPG, PNG, WebP. Maksimal 4MB. Biarkan kosong jika tidak ingin mengubah.</p>
...
<script>
    ...
    const initProvince    = "{{ $storedProvince }}";
    const initCity        = "{{ $storedCity }}";
    ...
    if (initProvince) populateCities(initProvince, initCity);

    // ===== Leaflet Map =====
    const defaultLat = -7.0534, defaultLng = 107.6418;
    const coordInput = document.getElementById('coordinates-input');
    let initLat = defaultLat, initLng = defaultLng, initZoom = 10;

    const existingCoords = coordInput.value;
    if (existingCoords) {
        const parts = existingCoords.split(',').map(s => parseFloat(s.trim()));
        if (parts.length === 2 && !isNaN(parts[0]) && !isNaN(parts[1])) {
            initLat = parts[0]; initLng = parts[1]; initZoom = 14;
        }
    }
    ...
    let marker = null;
    if (existingCoords && initLat !== defaultLat) {
        marker = L.marker([initLat, initLng])
            .addTo(map)
            .bindPopup(`<b>{{ $garden->kebun_name }}</b><br>${initLat.toFixed(6)}, ${initLng.toFixed(6)}`)
            .openPopup();
    }
