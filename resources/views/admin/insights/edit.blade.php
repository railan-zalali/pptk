    ...
    <form action="{{ route('admin.insights.update', $insight) }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input name="insight_type" type="text" required class="w-full border rounded px-3 py-2" value="{{ old('insight_type', $insight->insight_type) }}">
            </div>
            <div class="md:col-span-2">
                <label class="block text-sm font-medium text-gray-700 mb-2">Judul *</label>
                <input name="title" type="text" required class="w-full border rounded px-3 py-2" value="{{ old('title', $insight->title) }}">
            </div>
            <div class="md:col-span-2">
                <label class="block text-sm font-medium text-gray-700 mb-2">Deskripsi</label>
                <textarea name="description" rows="3" class="w-full border rounded px-3 py-2">{{ old('description', $insight->description) }}</textarea>
            </div>
            <div class="md:col-span-2">
                <label class="block text-sm font-medium text-gray-700 mb-2">Pesan *</label>
                <textarea name="message" rows="3" required class="w-full border rounded px-3 py-2">{{ old('message', $insight->message) }}</textarea>
            </div>
            <div>
                ...
                <select name="alert_level" required class="w-full border rounded px-3 py-2">
                    @foreach (['low','medium','high'] as $level)
                        <option value="{{ $level }}" {{ $insight->alert_level === $level ? 'selected' : '' }}>{{ ucfirst($level) }}</option>
                    @endforeach
                </select>
            </div>
            <div class="md:col-span-2">
                    ...
                    @if (is_array($recs) && count($recs))
                        @foreach ($recs as $rec)
                            <div class="flex gap-2">
                                <input name="recommendations[]" type="text" class="flex-1 border rounded px-3 py-2" value="{{ $rec }}">
